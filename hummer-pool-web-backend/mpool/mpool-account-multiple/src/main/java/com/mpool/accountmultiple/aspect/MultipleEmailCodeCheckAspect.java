...
@Aspect
@Component
public class MultipleEmailCodeCheckAspect extends BaseAspect {
    private static final Logger log = LoggerFactory.getLogger(MultipleEmailCodeCheckAspect.class);
    ...
    @Pointcut("@annotation(com.mpool.accountmultiple.aspect.annotation.MultipleCheckCodeEmail)")
    public void pointcutName() {
    }

    @Around("pointcutName()")
    public Object object(ProceedingJoinPoint joinPoint) throws Throwable {
        log.debug("Multiple check email code ");
        // 获取访问目标方法
        ...
        log.debug("Multiple cache email key --> {}", key);
        getCacheCode(key);
    ...
    private String getRequestCode() {
        ...
        if (StringUtils.isEmpty(code)) {
            ...
            if (StringUtils.isEmpty(code)) {
                throw new MpoolException("common.email.code.notfound", "请输入邮箱验证码");
            }
        }
        return code;
    }
