...
@Aspect
@Component
public class SmsCodeCheckAspect extends BaseAspect {

	private static final Logger log = LoggerFactory.getLogger(SmsCodeCheckAspect.class);

	@Pointcut("@annotation(com.mpool.common.cache.aspect.annotation.CheckCodeSms)")
	public void pointcutName() {
	}

	private final static String CODE_PARAM = "share_phoneCode";

	private final static String PHONE_PARAM = "share_phone";
	@Autowired
	private PhoneCodeCahceService phoneCodeCahceService;

	@Around("pointcutName()")
	public Object object(ProceedingJoinPoint joinPoint) throws Throwable {
		log.debug("check phone code ");
		// 获取访问目标方法
	...
	private String getRequestCode() {
		...
		if (StringUtils.isEmpty(code)) {
			throw new MpoolException("common.phone.code.notfound", "手机短信验证码错误");
		}
		return code;
	}
	...
	private String getCacheCode(String key) {
		...
		if (code == null) {
			log.error("sms code error, key={}", key);
