...
//统计
App.controller('shareStasticController', ['$scope', '$http', '$location', '$cookieStore', '$state', '$timeout', '$window','$translate','$stateParams',
	function($scope, $http, $location, $cookieStore, $state, $timeout, $window,$translate,$stateParams) {
		$scope.href = function(url){
			if(url == '') {
				$location.path('share/' + $stateParams.comId)
			}else{
				console.log('share/' + $stateParams.comId + '/'+url)
		...
		$scope.getStatistics = function(current,size){
			...
			$http.get(mpoolUI_url + '/pool/found/blocks?current='+current+'&size='+size + '&currencyName=' +$scope.cointype)
