...
//用户中心     -----   修改手机  提交流程
App.controller('modifyPhoneController', ['$scope', '$http', '$location', '$translate', '$state', '$timeout', '$interval', 'ngDialog', 'Notify', '$stateParams', 
	function($scope, $http, $location, $translate, $state, $timeout, $interval, ngDialog, Notify, $stateParams) {
		...
		$scope.phoneParameter = $stateParams.modifyuserPhone;   //手机参数
		
		//步骤流程
		$scope.show1 = true;
		$scope.initStepPhone = function (obj) {
			$(".steps").step({
			  	stepNames: [$translate.instant('notice.VerPh'),$translate.instant('notice.chgP'), $translate.instant('notice.Done')],
			  	initStep: obj
			})
			if(obj == 1){
				$scope.show1 = true;
