...
//用户中心     -----   修改邮箱  提交流程
App.controller('modifyEmailController', ['$scope', '$http', '$location', '$translate', '$state', '$timeout', '$interval', 'ngDialog', 'Notify', '$stateParams', 
	function($scope, $http, $location, $translate, $state, $timeout, $interval, ngDialog, Notify, $stateParams) {
		...
		$scope.emailParameter = $stateParams.modifyuserEmail;   //邮箱参数
		
		//步骤流程
		$scope.show1 = true;
		$scope.initStep = function (obj) {
			$(".steps").step({
			  	stepNames: [$translate.instant('notice.Verem'),$translate.instant('notice.chgem'),  $translate.instant('notice.Done')],
			  	initStep: obj
			})
			if(obj == 1){
				$scope.show1 = true;
...
//------------------------------第一步   邮箱验证			
		...
		$scope.getCodeEmail = function(){
			$http.get(mpoolUI_url + '/userManager/checkSecurityCode?securityCode='+ $scope.code_Email)
				.success(function(data){
					if(data.msg == 'ok'){
		...
		$scope.changePhone = function(){
			if($scope.code_Email == undefined || $scope.code_Email == ""){
				$scope.sub0 = false;
			}else{
				$scope.sub0 = true;
			}
